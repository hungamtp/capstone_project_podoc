package com.capstone.pod.constant.order;

public final class OrderSuccessMessage {
    public static final String ORDER_SUCCESS = "Order successfully";
    public static final String DELETE_ORDER_SUCCESS = "Delete order successfully";
    public static final String GET_SHIPPING_INFO_SUCCESS = "Get all my shipping information successfully";
    public static final String UPDATE_ORDER_DETAIL_STATUS_SUCCESS = "Update order detail status successfully";


}
